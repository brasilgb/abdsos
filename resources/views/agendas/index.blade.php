                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb pt-1 pb-1 float-right bg-transparent">
                        ...
                        <li class="breadcrumb-item active">
                            @if (isset($busca)) <a href="{{ route('agendas.index') }}">Agendamentos</a> / Busca @else
                            Agendamentos
                            @endif
                        </li>
                    </ol>
            ...
            <table id="tb-tarefas" class="table table-condensed table-striped mb-0">
                ...
                <tr>
                    ...
                    <td class="align-middle"><span
                            class="{{ $bgcolor($agenda->status) }} w-100 py-1 text-white shadow-sm border border-white d-block text-center rounded">{{ $status($agenda->status) }}</span>
                    </td>
                ...
                <div class="alert alert-danger">
                    <i class="fa fa-exclamation-triangle"></i> Não há dados a carregar! <a
                        href="{{ route('agendas.index') }}" title="Listar agendas"><i class="fa fa-sync-alt"></i></a>
                </div>
                @endforelse
            </table>
            @if (count($agendas) > 1)
