...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        ...
        <form id="formemail" action="{{ route('emails.update', [$email->id_email]) }}" method="POST" autocomplete="off">
            @method('PUT')
            @csrf
            ...
            <div class="form-group row">
                <label class="col-sm-3 col-form-label" for=""><span class="text-danger">*</span>
                    Senha:</label>
                <div class="col-sm-7">
                    <input type="password" class="form-control" name="senha" value="{{ old('senha', $email->senha) }}">
                    @error('senha')
                    <div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> {{ $message }}
                    </div>
                    @enderror
